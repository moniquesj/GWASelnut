...
# Set wd
setwd("/gene enrichment analysis")
# Genes in LD-decay interval to SNPs under selection analysed all together

### Load Data
annotation <- readMappings(file = "CavTom2PMs-1.0_gene_universe.txt")
geneUniverse <- names(annotation) 
genesOfInterest <- read.table("list_all_genes_loci_selection.txt",header=FALSE)
